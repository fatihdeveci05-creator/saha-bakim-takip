...
- **Deploy**: `45.155.19.196` (VPS5) — **canlı, gerçek deploy yapıldı** (29.08). PM2 process `abb-kontrol`, `/var/www/abb-kontrol`, sadece `127.0.0.1:3000`'e bağlı (internete kapalı, SSH tüneliyle erişim). Squid+WireGuard (Sefirox proxy havuzu) aynı VPS'te dokunulmadan duruyor. Detay: hafıza `abb_kontrol_implementation`.
